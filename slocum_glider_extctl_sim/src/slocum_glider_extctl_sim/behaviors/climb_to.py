...
class ClimbToBehavior(BehaviorWithSubstates):
    ...
    class Starting(Substate):
        DESCRIPTION = 'Starting the climb'

        def compute_controls(self, x):
            x.c_pinger_on = 10

        def next_state(self, parent, x):
            return parent.Climbing(parent)

    class Climbing(Substate):
    ...
    SUBSTATES = [Uninited, Waiting, None, Starting, Climbing, Complete]
